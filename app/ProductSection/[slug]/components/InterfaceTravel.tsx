...
export default function InterfaceTravel(props: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      >
        <ArrowLeft size={16} /> {t("products.minor.travelBackTo", { name: product.name })}
      </button>
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white p-8 rounded-[35px] shadow-xl shadow-slate-200/50 border border-slate-100">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-black uppercase italic text-slate-800">{t("products.minor.travelOrderData")}</h2>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{t("products.minor.travelActiveNumberNotice")}</p>
              </div>
            </div>
            
            <div className="space-y-4">
              <label className="text-xs font-black uppercase italic text-slate-500 ml-2">{t("products.minor.travelPhoneLabel")}</label>
              <input 
                type="text" 
                placeholder={t("products.minor.travelPhonePlaceholder")}
          ...
          <div className="bg-slate-900 p-6 rounded-[35px] text-white flex items-center justify-between shadow-2xl shadow-blue-900/20">
            <div>
              <p className="text-[10px] font-bold uppercase opacity-60">{t("products.financial.totalPayment")}</p>
