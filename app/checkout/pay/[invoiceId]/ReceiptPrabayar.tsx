  ...
  const handleDownloadImage = async () => {
    if (receiptRef.current && order) {
      try {
        ...
        link.href = dataUrl;
        link.click();
      } catch (err) { alert(t("receipt.saveImageFailed")); }
    }
  };
      ...
      >
        ...
        <div className="text-center border-b border-dashed border-slate-200 pb-3 mb-3 sm:pb-4 sm:mb-4">
          ...
          <div className={`flex items-center justify-center gap-1 text-[8px] sm:text-[9px] font-bold uppercase mt-1 ${theme.color}`}>
            {theme.icon} {isPln ? t("receipt.tokenElectricity") : isGame ? t("receipt.topupGame") : t("receipt.pulsaData")}
          </div>
        </div>
        ...
        <div className="w-full text-left">
          ...
          <div className="space-y-1 sm:space-y-1.5">
            <div className="flex justify-between items-center w-full">
              <span className="text-slate-400 uppercase text-[7.5px] sm:text-[9px] tracking-widest leading-none">{t("receipt.date")}</span>
              <span className="font-bold text-slate-800 text-right text-[10px] sm:text-[13px]">{new Date(order.updated_at || order.created_at).toLocaleString(locale === "en" ? "en-US" : "id-ID")}</span>
            </div>
            ...
            <div onClick={handleCopyInvoice} className="flex justify-between items-center w-full cursor-pointer group py-0 sm:py-0.5 transition-all active:scale-95">
              <span className="text-slate-400 uppercase text-[7.5px] sm:text-[9px] tracking-widest leading-none">{t("receipt.invoiceNo")}</span>
            ...
            <div className="flex justify-between items-center w-full">
              <span className="text-slate-400 uppercase text-[7.5px] sm:text-[9px] tracking-widest leading-none">{t("receipt.method")}</span>
          ...
          <div className="space-y-2 sm:space-y-3 mb-3 sm:mb-4">
            <div className="text-center w-full">
              <span className="text-slate-400 uppercase block text-[7.5px] sm:text-[9px] tracking-widest">{t("receipt.product")}</span>
            ...
            {(isPln || (isGame && order.customer_name)) && (
              <div className="text-center w-full">
                <span className="text-slate-400 uppercase block text-[7.5px] sm:text-[9px] tracking-widest">{isPln ? t("receipt.customerName") : t("receipt.nickname")}</span>
            ...
            {isPln && (
              <div className="text-center w-full">
                <span className="text-slate-400 uppercase block text-[7.5px] sm:text-[9px] tracking-widest">{t("receipt.customerId")}</span>
            ...
            <div className="space-y-1.5 sm:space-y-2">
              {isPln && (
                <>
                  ...
                  <div className="flex justify-between items-center w-full">
                    <span className="text-slate-500 uppercase">{t("receipt.electricityAmount")}</span>
              ...
              {!isPln && (
                <>
                  <div className="flex justify-between items-start w-full">
                    <span className="text-slate-500 uppercase shrink-0">{t("receipt.serialNumber")}</span>
                    <span className="font-bold text-slate-800 text-right break-all ml-4 uppercase">{formattedToken}</span>
                  </div>
                  <div className="flex justify-between items-center w-full">
                    <span className="text-slate-500 uppercase">{isGame ? t("receipt.gameId") : t("receipt.destinationNumber")}</span>
              ...
              <div className="border-t border-slate-200 border-opacity-30 pt-1.5 sm:pt-2 space-y-1.5 sm:space-y-2">
                <div className="flex justify-between items-center w-full">
                  <span className="text-slate-500 uppercase">{t("receipt.productPrice")}</span>
                  <span className="font-bold text-slate-800">{formatRupiah(hargaProduk, locale)}</span>
                </div>
                {biayaLayanan > 0 && (
                  <div className="flex justify-between items-center w-full">
                    <span className="text-slate-500 uppercase">{t("receipt.serviceFee")}</span>
                    <span className="font-bold text-slate-800">{formatRupiah(biayaLayanan, locale)}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {isPln && (
            <div className="text-center py-4 sm:py-6">
              <p className="text-slate-400 uppercase text-[7.5px] sm:text-[9px] mb-1">{t("receipt.tokenStroomNumber")}</p>
        ...
        <div className={`border-t border-dashed border-slate-200 ${isPln ? 'pt-0' : 'pt-4'} mt-3`}>
          <div className={`flex justify-between items-center ${isPln ? 'bg-amber-600' : isGame ? 'bg-indigo-600' : 'bg-emerald-600'} text-white px-3 py-2.5 sm:px-4 sm:py-3 rounded-xl shadow-lg`}>
            <span className="font-bold italic uppercase text-[8px] sm:text-[10px]">{t("receipt.totalPaid")}</span>
            <span className="text-sm sm:text-lg font-black italic">{formatRupiah(totalLunas, locale)}</span>
          </div>
        </div>
        ...
        <div className="text-center mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-dashed border-slate-200 text-[7px] sm:text-[8px] text-slate-400 uppercase font-bold italic leading-relaxed">
          ...
          <p className={theme.color}>{t("receipt.saveProofNotice")}</p>
        </div>
      </div>

      {/* ACTION BUTTONS */}
      <div className="flex flex-row gap-2 w-full max-w-md mx-auto no-print px-1">
        <button onClick={() => window.print()} className="flex-1 bg-slate-900 text-white py-2 sm:py-2.5 rounded-xl font-black italic uppercase text-[9px] sm:text-[10px] flex items-center justify-center gap-1.5 hover:opacity-90 active:scale-95 shadow-md"><Printer size={14} /> {t("receipt.print")}</button>
        <button onClick={handleDownloadImage} className={`flex-1 ${isPln ? 'bg-amber-600' : isGame ? 'bg-indigo-600' : 'bg-emerald-600'} text-white py-2 sm:py-2.5 rounded-xl font-black italic uppercase text-[9px] sm:text-[10px] flex items-center justify-center gap-1.5 hover:opacity-90 active:scale-95 shadow-md`}><Download size={14} /> {t("receipt.save")}</button>
      </div>
