...
export async function POST(req: Request) {
  try {
    ...
    // 1. Ambil data produk berserta nama kategorinya
    ...
    const updates = products.map((p: any) => {
      ...
      const cost = Number(updatedProduct.cost || 0);
      ...
      if (disc > 0 && profitKotor > 0) {
         ...
         const randomPersen = (p.id.charCodeAt(0) % 6) + 15;
    ...
    for (let i = 0; i < updates.length; i += chunkSize) {
       ...
       const promises = chunk.map(upd => 
           supabaseAdmin.from('products').update(upd).eq('id', upd.id)
       );
