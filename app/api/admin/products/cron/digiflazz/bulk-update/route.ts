...
export async function GET(req: Request) {
  // 1. SATPAM ROBOT (Khusus Jalur Cron)
  ...
  try {
    // 2. AMBIL DATA STRATEGI & SETTINGS
    ...
    // 4. FUNGSI EKSEKUTOR (Logic Perhitungan Harga)
    ...
    // 5. EKSEKUSI BATCH UPDATE (Chunking per 50 baris)
    const chunkSize = 50;
    ...
    // 6. LOG AKTIVITAS KHUSUS ROBOT
    try {
      await supabaseAdmin.from('activity_logs').insert([{
        action: "AUTO BULK DIGIFLAZZ (CRON)",
        details: `Robot otomatis merapikan harga ${updateCount} produk Digiflazz sesuai strategi margin.`,
    ...
    return NextResponse.json({ success: true, message: `Robot Berhasil Update: ${updateCount} Produk.` });
