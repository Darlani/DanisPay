...
export default function TransactionHistoryModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-1000 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      {/* Container: Lebar diset ke max-w-md agar pas dan tidak terlalu kurus/lebar */}
      <div className="bg-white w-full max-w-md max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
        
        {/* HEADER */}
        <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
          <div className="flex items-center gap-3">
            {/* PERBAIKAN: Kunci ukuran kotak ikon jadi w-10 h-10 dan shrink-0 agar tidak melar */}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 custom-scrollbar">
          ...
          <div className="mt-6 flex flex-col items-center justify-center min-h-40">
            {order ? (
              <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                
                {/* AREA STRUK INVOICE */}
                <div ref={receiptRef} className="w-full bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative">
                  ...
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div className="text-left flex-1">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Official Receipt</p>
                      <p className="text-sm font-bold text-slate-900 leading-tight">DanisPay Digital</p>
                    </div>
                  </div>
              ...
              <div className="text-center py-6 animate-in fade-in">
                <AlertCircle className="mx-auto text-rose-500 mb-3" size={36} strokeWidth={1.5} />
                <p className="text-sm font-medium text-slate-600 px-4">{error}</p>
              </div>
            ) : (
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 text-center shrink-0">
          <p className="text-[11px] font-semibold text-slate-400 tracking-wide">DanisPay - Safe & Fast Transaction</p>
        </div>
