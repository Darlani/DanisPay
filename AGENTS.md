...
DaPay supports:
- `id` = Indonesian (DEFAULT, root URLs without prefix: `/`, `/news`, `/promo`)
- `en` = English (subpath URLs with `/en` prefix: `/en`, `/en/news`, `/en/promo`)

For all NEW public user-facing features:
- User-facing text MUST use i18n translation keys.
- Do NOT hardcode Indonesian or English UI strings inside reusable components when a translation key is appropriate.
- Every new public feature must add/update both translation dictionaries:
  - `messages/id.json`
  - `messages/en.json`
- Use `useTranslations()` / `t("namespace.key")` from `@/lib/i18n/context`.
- Forbidden: `"Beli Sekarang"` hardcoded in markup when a translation key should be used.
- Correct: `t("common.buyNow")` with matching entries in `id.json` and `en.json`.
- Do NOT require Admin/internal-only dashboards to become bilingual yet (Admin stays Indonesian initially).
- CMS editorial content (`public_contents`) is distinct from UI dictionary keys and will be localized in a separate dedicated phase.
