-- Migration: 20260916100000_sandbox_funnel_analytics_rpc.sql
-- Purpose: Phase 5B Server-Side Funnel & Retention Analytics RPC
-- 1. Pre-check: Ensure function does not already exist
-- 2. Create public.get_sandbox_funnel_analytics function
--    - Input parameters: p_event_start, p_event_end, p_cohort_start, p_cohort_end
--    - Computes 8-stage canonical funnel using COUNT(DISTINCT canonical_identity)
--    - Resolves identity stitching at query time (treating ambiguous anonymous IDs as unstitched)
--    - Distinguishes mature retention cohorts from immature cohorts in Asia/Jakarta timezone
--    - Reports first value from successful (Berhasil) sandbox_orders at/after activation
--    - Reports conversion from self-service sandbox_tester_conversion_events
--    - Aggregates attribution strictly on persisted source, medium, campaign
--    - Returns data quality anomalies
-- 3. Security: SECURITY DEFINER, restricted exclusively to service_role
...
BEGIN
  IF EXISTS (
    ...
    WHERE n.nspname = 'public' AND p.proname = 'get_sandbox_funnel_analytics'
  ) THEN
    RAISE EXCEPTION 'OBJECT_ALREADY_EXISTS: public.get_sandbox_funnel_analytics';
  END IF;
END $$;
...
BEGIN
  ...
  -- 3. Identity Stitching & Disambiguation
  anon_mapping_stats AS (
    SELECT
      anonymous_id,
      COUNT(DISTINCT user_id) AS linked_user_count,
      MIN(user_id) AS single_user_id
